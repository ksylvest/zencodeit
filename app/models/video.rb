class Video < ActiveRecord::Base
  ...
  DEFAULTS = {
    ...
    notifications: [
      {
        format: 'json',
        url: 'http://zencodeit.heroku.com/videos/notifications.json',
      },
    ],
  }.freeze
  ...
  before_save :zencodeit, if: ->(video) { video.encoding.changed? }
  ...
  def zencodeit
    ...
    OUTPUTS.each do |style, options|
      ...
      output = output.merge(url: encoding.url(style))
      outputs << output
    end
    ...
    if response.success?
      build_job(zencoder_id: response.body['id'])
